...
/*********************************************************     
**********************************************************/	 
#define DELAY_TIME  delay_us(50); //50
uint16_t Handkey;	// 按键值读取，零时存储。
uint8_t Comd[2]={0x01,0x42};	//开始命令。请求数据
uint8_t PS2_Data[9]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}; //数据存储数组
uint16_t MASK[]={
    PSB_SELECT,
    PSB_L3,
    PSB_R3 ,
...
void PS2_SetInit(void)
{
	...
	PS2_Vibration(0x68,0x68);
	PS2_ExitConfing();		  //完成并保存配置
}

//获取手柄数据函数
PSTwo_Data PS2_Receive (void)
{   
	PSTwo_Data ps_2_data;
	ps_2_data.PS2_KEY=PS2_DataKey();
	ps_2_data.PS2_LX=PS2_AnologData(PSS_LX);
	ps_2_data.PS2_LY=PS2_AnologData(PSS_LY);
	ps_2_data.PS2_RX=PS2_AnologData(PSS_RX);
	ps_2_data.PS2_RY=PS2_AnologData(PSS_RY);
	return ps_2_data;
}
