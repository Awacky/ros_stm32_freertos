...
uint8_t MPU6500::Write_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
    uint8_t i;
    Wirempu6500.Start();
    Wirempu6500.Send_Byte((addr<<1)|0); 	   //发送器件地址+写命令
    if(Wirempu6500.Wait_Ack_Reply())           //等待应答
    {
        Wirempu6500.Stop();
        return 1;
    }
    Wirempu6500.Send_Byte(reg);         	   //写寄存器地址
    Wirempu6500.Wait_Ack_Reply();              //等待应答
    for(i=0;i<len;i++)
    {
        Wirempu6500.Send_Byte(buf[i]);  	   //发送数据
        if(Wirempu6500.Wait_Ack_Reply())       //等待ACK
        {
            Wirempu6500.Stop();
            return 1;
        }
...
{ 
    ...
    Wirempu6500.Send_Byte((addr<<1)|0);   // IIC_Send_Byte((addr<<1)|0); //发送器件地址+写命令
    if(Wirempu6500.Wait_Ack_Reply())      //等待应答
    {
        Wirempu6500.Stop();
        return 1;
    ...
    Wirempu6500.Send_Byte(reg);         		//写寄存器地址
    Wirempu6500.Wait_Ack_Reply();				//IIC_Wait_Ack();//等待应答
	Wirempu6500.Start();						//IIC_Start();                
    Wirempu6500.Send_Byte((addr<<1)|1); 		//发送器件地址+读命令
    Wirempu6500.Wait_Ack_Reply();               //等待应答
    while(len)
    {
        if(len==1)
			*buf=Wirempu6500.Read_Byte_Ack(0);	//读数据,发送nACK 
		else 
			*buf=Wirempu6500.Read_Byte_Ack(1);  //读数据,发送ACK  
		len--;
		buf++;  
    }
    Wirempu6500.Stop();                 		//产生一个停止条件
    return 0;       
}
...
uint8_t MPU6500::Write_Byte(uint8_t addr,uint8_t reg,uint8_t data)
{
	Wirempu6500.Start();
    Wirempu6500.Send_Byte((addr<<1)|0); 	   //发送器件地址+写命令
    if(Wirempu6500.Wait_Ack_Reply())           //等待应答,如果返回1失败则停止
    {
        Wirempu6500.Stop();
        return 1;
    }
    Wirempu6500.Send_Byte(reg);         	   //写寄存器地址
    Wirempu6500.Wait_Ack_Reply();              //等待应答
    Wirempu6500.Send_Byte(data);       	 	   //发送数据
    if(Wirempu6500.Wait_Ack_Reply())           //等待ACK，如果返回1失败则停止
    {
        Wirempu6500.Stop();
        return 1;
    }
...
uint8_t MPU6500::Read_Byte(uint8_t addr,uint8_t reg)
{
    ...
    Wirempu6500.Send_Byte((addr<<1)|1); 		//发送器件地址+读命令
    Wirempu6500.Wait_Ack_Reply();               //等待应答
    res=Wirempu6500.Read_Byte_Ack(0);			//读数据,发送nACK  
    Wirempu6500.Stop();                         //产生一个停止条件
    return res;  
}