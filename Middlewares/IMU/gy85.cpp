...
imu_check Gy85::check(void)
{
	imu_check is_imu_check;
	is_imu_check.acc = 1;
	is_imu_check.gyro = 1;
	is_imu_check.magn = 1;
    is_imu_check.acc = check_accelerometer();
	is_imu_check.gyro = check_gyroscope();
	is_imu_check.magn = check_magnetometer();
	return is_imu_check;

}
int Gy85::check_gyroscope()
{
...
geometry_msgs::Vector3 Gy85::readAccelerometer(void)
{
	...
	accel.x =  ((float)ACC_X_INVERT*(int16_t)((int)acc_buffer[2*ACC_X_AXIS+1]<<8 | acc_buffer[2*ACC_X_AXIS]) / ADXL345_SCALE);
	accel.y =  ((float)ACC_Y_INVERT*(int16_t)((int)acc_buffer[2*ACC_Y_AXIS+1]<<8 | acc_buffer[2*ACC_Y_AXIS]) / ADXL345_SCALE);
	accel.z =  ((float)ACC_Z_INVERT*(int16_t)((int)acc_buffer[2*ACC_Z_AXIS+1]<<8 | acc_buffer[2*ACC_Z_AXIS]) / ADXL345_SCALE);
	return accel;
}
geometry_msgs::Vector3 Gy85::readGyroscope(void)
{
	...
	gyro.x = (float)(GYRO_X_INVERT*(int16_t)(((int)gyro_buffer[2*GYRO_X_AXIS] <<8) | gyro_buffer[2*GYRO_X_AXIS+1])) * ITG3205_SCALE;  //rad/s
	gyro.y = (float)(GYRO_Y_INVERT*(int16_t)(((int)gyro_buffer[2*GYRO_Y_AXIS] <<8) | gyro_buffer[2*GYRO_Y_AXIS+1])) * ITG3205_SCALE;
	gyro.z = (float)(GYRO_Z_INVERT*(int16_t)(((int)gyro_buffer[2*GYRO_Z_AXIS] <<8) | gyro_buffer[2*GYRO_Z_AXIS+1])) * ITG3205_SCALE;
	return gyro;
}
geometry_msgs::Vector3 Gy85::readMagnetometer(void)
{
	...
	mag.x =  (float)(MAG_X_INVERT * ((int16_t)((int)mag_buffer[2*MAG_X_AXIS] << 8) | (mag_buffer[2*MAG_X_AXIS+1]))) * HMC5883L_MAG_SCALE;
	mag.y =  (float)(MAG_Y_INVERT * ((int16_t)((int)mag_buffer[2*MAG_Y_AXIS] << 8) | (mag_buffer[2*MAG_Y_AXIS+1]))) * HMC5883L_MAG_SCALE;
	mag.z =  (float)(MAG_Z_INVERT * ((int16_t)((int)mag_buffer[2*MAG_Z_AXIS] << 8) | (mag_buffer[2*MAG_Z_AXIS+1]))) * HMC5883L_MAG_SCALE;
    if(mag.x == 0.0 )
        mag.x = -28514.00;
	if(mag.y == 0.0)
        mag.y = -28302.00;
