...
#define UBX_RX_NAV_PVT_FLAGS_GNSSFIXOK    0x01 /**< gnssFixOK (A valid fix (i.e within DOP & accuracy masks)) gnssFixOK（有效修复（即在DOP和精度掩码内*/
...
void reset_ubx_decoder(ubx_decoder_t* ubx_decoder);																		//复位检测的结构体数据
uint8_t init_ubx_decoder(ubx_decoder_t* ubx_decoder, ubx_rx_handle_ptr ubx_rx_handle);									//初始化检测的结构体数据
int parse_ubx_char(ubx_decoder_t* ubx_decoder, const uint8_t c);														//解析数据，轮询的方式

uint8_t send_ubx_msg(ubx_decoder_t* ubx_decoder, const uint16_t msg, const uint8_t* payload, const uint16_t length);	//发送MSG数据
uint8_t configure_ubx_msg_rate(ubx_decoder_t* ubx_decoder, const uint16_t msg, const uint8_t rate);						//配置GPS的数据发送频率

void registeUbxSend(void(*Send_t)(uint8_t *src_t,uint16_t srcLen));														//注册bux发送函数，具体实现方法
